...
function encodeMessage() {
  ...
  // Step 3: Obfuscate the binary
  var obfuscatedBinaryMessage = obfuscateBinary(binaryMessage);
  $('.binary_Obf_textarea').text(obfuscatedBinaryMessage); // Show obfuscated binary

  // Step 4: Hide obfuscated binary into the image
...
function decodeMessage() {
  ...
  var output = "";
  for (var i = 0; i < cleanBinaryMessage.length; i += 8) {
      var c = 0;
      for (var j = 0; j < 8; j++) {
          c <<= 1;
          c |= parseInt(cleanBinaryMessage[i + j]);
      }
      output += String.fromCharCode(c);
  }

  // Step 4: Decrypt using key
  var key = $("input[name=key]").val();
